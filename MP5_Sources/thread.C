...
static void thread_shutdown() {
   ...
   Console::puts("Ready for shutdown\n");


// here we want to disable the interrups
    if(Machine::interrupts_enabled()){
     Machine::disable_interrupts();
    }
   
    // put thread back to queue and terminate it 
    SYSTEM_SCHEDULER->resume(current_thread);
    SYSTEM_SCHEDULER->terminate(current_thread);

// call memory pool to release frames -- using stack address
    MEMORY_POOL->release((unsigned long) (current_thread->get_stackaddress()));
    MEMORY_POOL->release((unsigned long) current_thread);
   
   current_thread=0;
// call scheduler to pass cpu to another thread from the queue in FIFO order
...
// as given by above hints, here we want to enable interrupts to mark thread start
...
unsigned long Thread::get_stackaddress(){
