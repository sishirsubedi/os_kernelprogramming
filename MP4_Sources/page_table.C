...
// similarly page table aslo needs one frame from the process mem pool
...
//now mark last entry of the page directory to point to itself
page_directory[1023] = (unsigned long) page_directory |3;


//initialize all the 20 pt_vmpools to point to null
...
unsigned long page_enable = read_cr0();


page_enable = page_enable | 0x80000000;

write_cr0(page_enable);
...
// Console::puti(_r->err_code); // it is zero


VMPool ** temp_pt_vmpools = current_page_table->pt_vmpools;

unsigned int found = 0;

unsigned long vm_pf_index=0;

for(unsigned int i=0; i<VM_SIZE;i++){
	
 // if(temp_pt_vmpools[i]== NULL)
   //{
    //break;
   //}
   	if(temp_pt_vmpools[i]->is_legitimate(cr2_message))
   	      {
		found = 1;
        vm_pf_index = i;
        //break;
...
if((temp_page_directory[page_directory_index] & 1 ) == 0 ){
 ...
 //    temp_page_table = (unsigned long *) (0xFFC00000 | process_mem_pool->get_frames(1)*PAGE_SIZE);
 ...
 temp_page_directory[page_directory_index] = (unsigned long) ((process_mem_pool->get_frames(1)*PAGE_SIZE)|3);

 temp_page_table = (unsigned long *)((temp_page_directory[page_directory_index])& 0xFFC00000);

 for (int i = 0; i<ENTRIES_PER_PAGE;i++){
 	temp_page_table[i] = 0;
 }

 temp_page_table[page_table_index & 0x3FF]= (PageTable::process_mem_pool->get_frames(1)*PAGE_SIZE)|3;


//Console::puts("I dont have page ..help \n");
...
if((temp_page_table[page_table_index] & 1 ) == 1) {
...
//temp_page_directory[page_directory_index]= (unsigned long) (page_to_enter) | 3;

//Console::puts("I have page but not page entry..help \n");
...
void PageTable::register_pool(VMPool * _pool){
     ...
     // mark the index to register
        int registering_index=-1;
      //go through this page tables vmpools and check the empty index  
        for (unsigned int i=0;i<VM_SIZE;++i){

            if (pt_vmpools[i]==NULL){
                registering_index=i; // found empty index
            }
        }
        if (registering_index>=0){
            pt_vmpools[registering_index]=_pool; // register the pool
            Console::puts("registering vmpool is successful\n");
        }
        else
            Console::puts("error in registering pool \n");

}
    
void PageTable::free_page(unsigned long _page_no){

	// get the page directory index as input

unsigned long temp_page_directory_index = _page_no >> 22;

// get the page table index

unsigned long temp_page_index = (_page_no >> 12) & 0x3FF;

// get the page table pointer

unsigned long* temp_page_table = (unsigned long*)(0xFFC00000 | (temp_page_directory_index << 12));

// get the frame number using page table index to release

unsigned long frame_number= temp_page_table[temp_page_index];

// call process memory pool to release this frame number
